...
public class ChatControll {
	...
	public ModelAndView popupChattingGet(ModelAndView mv, int pd_num, HttpServletRequest request, String buyer) {
		// 로그인된 회원정보 가져오기
		...
		if(member == null) {
			...
			ChattingVo checkChattingRoom = new ChattingVo();
			if(chatProduct.getPd_mb_id().equals(member.getMb_id())) {
				// pd_num, mb_id, buyer와 일치하는 채팅룸이 있는지 확인후 가져오기
				checkChattingRoom = chatService.getSellerChatRoom(pd_num, member.getMb_id(), buyer);
			}
			// pd_mb_id와 현재 로그인 된 mb_id가 일치하지 않는다면
			else {			
				// pd_num, pd_mb_id, mb_id와 일치하는 채팅룸이 있는지 확인후 가져오기
				checkChattingRoom = chatService.getCheckChattingRoom(pd_num, chatProduct.getPd_mb_id(), member.getMb_id());			
			}
	...
	@ResponseBody
	public Object sendMessagePost(@RequestBody @RequestParam String chmg_content, @RequestParam Integer pd_num,  @RequestParam String pd_mb_id, 
			@RequestParam String opponent, HttpServletRequest request) {		
