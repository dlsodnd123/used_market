...
<body>
	...
	<c:if test="${member == null }">
    	<div class="container chatting-error">
	    	<div class="chatting-error-message"><i class="fas fa-exclamation-triangle"></i> 잘못된 접근 방식입니다.</div>
    	</div>
	</c:if>
</body>
<script>
 	// 1초마다 새로고침
 	if($('input[id=chmg_num]').last().val() != null){
	 	setInterval(() => {
			var pd_num = '${pd_num}';
	...
	if(lastOpponentLoc != null || lastMySelfLoc != null){
		// 상대방이 메시지가 마지막이라면
